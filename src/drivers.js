...
export default function Dashboard() {
  ...
  const available = drivers.filter(
    (d) => d.computed_status === "AVAILABLE"
  ).length;

  const onTrip = drivers.filter(
    (d) => d.computed_status === "ONTRIP"
  ).length;

  const busy = drivers.filter(
    (d) => d.computed_status === "BUSY"
  ).length;
  ...
  return (
    <div className="p-5 bg-[#f6f7fb] min-h-screen">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        ...
        <div className="bg-white border border-[#e5e7eb] rounded-xl p-3">
          <p className="text-[11px] text-[#6b7280]">Available</p>
          <h2 className="text-xl font-bold text-green-600">{available}</h2>
        </div>

        <div className="bg-white border border-[#e5e7eb] rounded-xl p-3">
          <p className="text-[11px] text-[#6b7280]">On Trip</p>
          <h2 className="text-xl font-bold text-red-600">{onTrip}</h2>
        </div>

        <div className="bg-white border border-[#e5e7eb] rounded-xl p-3">
          <p className="text-[11px] text-[#6b7280]">Busy</p>
          <h2 className="text-xl font-bold text-yellow-600">{busy}</h2>
        </div>
      </div>
